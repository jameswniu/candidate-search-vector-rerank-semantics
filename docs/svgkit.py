...
Theme note: this repo's accent is VIOLET (the vector-database identity of the
project); BLUE stays for retrieval stages, ORANGE for LLM reranking, and AQUA
for verified/passing states, so the diagrams read the same way across repos.
"""
...
VIOLET, VIOLET_T = "#8a63d2", "#ab8ce6"
THEME, THEME_T = VIOLET, VIOLET_T
...
<defs>
  ...
  <linearGradient id="rl{uid}" x1="0" y1="0" x2="1" y2="0">
    <stop offset="0" stop-color="{THEME}" stop-opacity="0.95"/><stop offset="1" stop-color="{THEME}" stop-opacity="0"/>
  </linearGradient>
  ...
  <marker id="arv{uid}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="{VIOLET}"/></marker>
</defs>
...
def title_block(uid, eyebrow, title, x=36):
    return (f'<text x="{x}" y="42" fill="{THEME}" font-size="15" font-weight="700" letter-spacing="3" font-family="{MONO}">{eyebrow}</text>\n'
